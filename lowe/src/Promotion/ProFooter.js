    ...
    clickShare3 = () => {
        if (navigator.share) {
            navigator.share({
                title: '로위 - 헤어스타일 스토어',
                text: '온라인 헤어스타일 스토어. 내가 원하는 모든 헤어스타일 부터 예약까지 한 번에, 로위(LOWE)',
                url: "https://lowehair.kr/event_review01",
            });
        } else {
            navigator.clipboard.writeText("https://lowehair.kr/event_review01")
                .then(() => {
                    alert("링크복사 완료 주변에 알려주세요!");
                })
        }
        ...
        return (
            <>
                {this.props.footer === "promotion1" ?
                    ...
                    this.props.footer === "promotion3" ?
                        <div className="ProFooter_footer3">
                            <div>
                                <div style={{ lineHeight: "62px" }} >
                                    <a href="/myreviews" style={{color: "#ffffff"}}>
                                        리뷰참여
                                    </a>
                                </div>
                                <div style={{ lineHeight: "62px", backgroundColor: "#ffffff" }} onClick={this.clickShare3}>
                                    공유하기
                                </div>
                            </div>
                        </div> :
